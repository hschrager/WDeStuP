...
class ReachAllPlaces(PluginBase):
    def main(self):
        ...
        def workflow_net(nodes, place_elements, graph):
            wn = False
            source = find_source(place_elements, graph)
            sink = find_sink(graph)
            if source is None or sink is None:
                return False
            else:
                wn = find_path(source, graph, nodes)
            return wn

        def find_source(place_elements, graph):
        ...
        def find_path(source, graph, nodes):
            valid_path = True
            visited = []
            queue = []
        ...
        if workflow_net(nodes, place_elements, graph):
            classifications.append("Workflow net")
        logger.info('This Petrinet is of type: ${}'.format(classifications))

        self.create_message(active_node, 'This Petrinet is of type: ${}'.format(classifications), severity='info')